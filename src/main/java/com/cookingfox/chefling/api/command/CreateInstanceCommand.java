...
public interface CreateInstanceCommand {
     ...
     * instance is not stored (that's what {@link CheflingContainer#getInstance(Class)} is for), so
     * only use this method directly when you need a NEW instance. It uses the type mappings (from
     * the `map...` methods) to create the instance. If no mapping is available, it attempts to
     * resolve the dependencies by inspecting the constructor parameters. If the created instance
     * implements {@link CheflingLifecycle}, its {@link CheflingLifecycle#initialize()} method will
     * be called.
     *
